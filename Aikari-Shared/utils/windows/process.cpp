...
void killProcessByName(const std::string& procNameASCII)
{
    ...
    try
    {
        ...
        );  // 没必要再去调那什么 StringToWString 了, 进程名一般不会是非 ASCII
            // 字符 (至少这个场景里)
    }
    catch (...)
    {
        throw std::runtime_error(
            "Invalid procName provided, only ASCII process name accepted."
        );
    }
    ...
    LOG_INFO("Searching for desired process to kill: {}", procNameASCII);
