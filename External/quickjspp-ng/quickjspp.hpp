...
// Credit: @std-microblock
// From:
// https://github.com/std-microblock/breeze-shell/blob/master/src/shell/script/quickjspp.hpp
...
namespace qjs
{
     ...
     * Describes how to convert type R to/from JSValue. Second template argument
     * can be used for SFINAE/enable_if type filters.
     */
    ...
    struct js_traits
    {
         ...
         * @param v This value is passed as JSValueConst so it should be freed
         * by the caller.
    ...
    struct js_traits<std::variant<Ts...>>
    {
        ...
        /** Attempt to match common types (integral, floating-point, string,
         * etc.) */
        template <
            template <typename R> typename Trait,
            typename U,
            typename... Us>
        static std::optional<std::variant<Ts...>> unwrapImpl(
        ...
        static std::optional<std::variant<Ts...>> unwrapObj(
            JSContext* ctx, JSValueConst v, JSClassID class_id
        )
        {
            ...
            // try to unwrap embedded variant (variant<variant<...>>), might be
            // slow
            ...
            if constexpr (is_vector<U>::value)
            {
                if (JS_IsArray(ctx, v) == 1)
                {
                    ...
                    bool ok =
                        isCompatible<std::decay_t<typename U::value_type>>(
                            ctx, firstElement
                        );
            ...
            if constexpr (is_pair<U>::value)
            {
                if (JS_IsArray(ctx, v) == 1)
                {
                    ...
                    bool ok =
                        isCompatible<std::decay_t<typename U::first_type>>(
                            ctx, firstElement
                        ) &&
        ...
        /** Attempt to cast to types satisfying traits, ordered in terms of
         * priority
         */
        template <
            template <typename T> typename Trait,
            template <typename T> typename... Traits>
        static std::variant<Ts...> unwrapPriority(
            JSContext* ctx, JSValueConst v
        )
        {
            ...
            JS_ThrowTypeError(
                ctx, "Expected type %s", QJSPP_TYPENAME(std::variant<Ts...>)
            );
            throw exception{ ctx };
        }
        ...
        static std::variant<Ts...> unwrap(JSContext* ctx, JSValueConst v)
        {
            ...
            switch (tag)
            {
                ...
                case JS_TAG_OBJECT:
                    if (auto result =
                            unwrapObj<Ts...>(ctx, v, JS_GetClassID(v)))
                    {
                        return *result;
                    }
                    JS_ThrowTypeError(
                ...
                default:  // more than JS_TAG_FLOAT64 (nan boxing)
                    return unwrapPriority<is_double, std::is_floating_point>(
                        ctx, v
                    );
            }
    ...
    {
        static JSValue wrap(
            JSContext* ctx, const std::weak_ptr<T>& value
        ) noexcept
        {
    ...
    namespace detail
    {
         ...
         */
        template <typename... Args>
        std::tuple<std::decay_t<Args>...> unwrap_args(
        ...
        JSValue wrap_call(
            JSContext* ctx, Callable&& f, int argc, JSValueConst* argv
        ) noexcept
        {
            try
            {
        ...
        {
            ((argv[I] =
                  js_traits<std::decay_t<std::tuple_element_t<I, Tuple>>>::wrap(
                      ctx, std::get<I>(tuple)
                  )),
             ...);
        }
        ...
        using class_from_member_pointer_t =
            typename class_from_member_pointer<T>::type;
    ...
    template <
        auto F,
        bool PassThis = false /* pass this as the first argument */>
    struct fwrapper
    {
            ...
            return JS_NewCFunction(
                   ...
                   JSValueConst* argv) noexcept -> JSValue
                {
                    if constexpr (PassThis)
                        ...
                        return detail::wrap_call<R, Args...>(
                            ctx, F, argc, argv
                        );
                },
                fw.name,
    ...
    struct js_traits<fwrapper<F, PassThis>>
    {
        static JSValue wrap(JSContext* ctx, fwrapper<F, PassThis> fw) noexcept
        {
            return JS_NewCFunction(
                ...
                {
                    return detail::
                        wrap_this_call<R, std::shared_ptr<T>, Args...>(
                            ctx, F, this_value, argc, argv
                        );
                },
                fw.name,
     ...
     * PassThis is ignored and treated as true */
    template <
        typename R,
        class T,
        typename... Args,
    ...
    {
        static_assert(
            std::is_constructible<T, Args...>::value, "no such constructor!"
        );
    ...
    namespace detail
    {
        ...
        inline JSValue GetPropertyPrototype(
            JSContext* ctx, JSValueConst this_value
        )
        {
            // constant atom: doesn't need to be freed and doesn't change with
            // context
            static const JSAtom JS_ATOM_prototype =
                JS_NewAtom(ctx, "prototype");
    ...
    {
        static JSValue wrap(
            JSContext* ctx, ctor_wrapper<T, Args...> cw
        ) noexcept
        {
            return JS_NewCFunction2(
                   ...
                   JSValueConst* argv) noexcept -> JSValue
                {
                    ...
                    {
#if defined(__cpp_rtti)
                        // automatically register class on first use (no
                        // prototype)
                        ...
                        JS_ThrowTypeError(
                            ctx,
                            "quickjspp ctor_wrapper<T>::wrap: Class is not "
                            "registered"
                        );
                        return JS_EXCEPTION;
#endif
                    ...
                    try
                    {
                        ...
                        JS_SetOpaque(
                            jsobj, new std::shared_ptr<T>(std::move(ptr))
                        );
                        return jsobj;
                    }
                    catch (exception)
                    ...
                    // return detail::wrap_call<std::shared_ptr<T>,
                    // Args...>(ctx, std::make_shared<T, Args...>, argv);
                },
                cw.name,
                sizeof...(Args),
    ...
    /** Conversions for std::shared_ptr<T>. Empty shared_ptr corresponds to
     * JS_NULL. T should be registered to a context before conversions.
     * @tparam T class type
     */
    template <class T>
    struct js_traits<std::shared_ptr<T>>
    {
        ...
        /// Used by registerDerivedClass to register new derived classes with
        /// this class' base type.
        ...
        inline static std::unordered_map<JSClassID, ptr_cast_fcn_t>
            ptrCastFcnMap;
         ...
         * @param ptr_cast_fcn function to obtain a std::shared_ptr from the
         * JSValue
         */
        template <typename D>
        static void registerDerivedClass(
            JSClassID derived_class_id, ptr_cast_fcn_t ptr_cast_fcn
        )
        {
            static_assert(
            ...
            // Register how to obtain the std::shared_ptr from the derived
            // class.
        ...
        static std::enable_if_t<
            !std::is_same_v<B, T> && !std::is_same_v<B, void>>
        ensureCanCastToBase(JSContext* ctx)
        {
            static_assert(
                std::is_base_of_v<B, T>, "Type is not a derived class"
            );
        ...
        {
            ensureCanCastToBase<
                detail::class_from_member_pointer_t<decltype(M)>>(ctx);
        }
         ...
         * @param exotic pointer to QJS exotic methods(static lifetime) which
         * allow custom property handling. see quickjs doc
         * @throws exception
         */
         ...
         * Creates an object with class if #QJSClassId and sets its opaque
         * pointer to a new copy of #ptr.
         */
        static JSValue wrap(JSContext* ctx, std::shared_ptr<T> ptr)
        {
            ...
            if (QJSClassId == 0)  // not registered
            {
                ...
                JS_ThrowTypeError(
                    ctx,
                    "quickjspp std::shared_ptr<T>::wrap: Class is not "
                    "registered"
                );
                return JS_EXCEPTION;
#endif
    ...
    struct js_traits<T*, std::enable_if_t<std::is_class_v<T>>>
    {
        static JSValue wrap(JSContext* ctx, T* ptr)
        {
                ...
                0)  // not registered
            {
                ...
                // it probably means you are passing JSValueConst to where
                // JSValue is expected
                js_traits<std::shared_ptr<T>>::register_class(
                    ctx, typeid(T).name()
                );
                ...
                JS_ThrowTypeError(
                    ctx,
                    "quickjspp js_traits<T *>::wrap: Class is not registered"
                );
                return JS_EXCEPTION;
#endif
            }
            auto jsobj = JS_NewObjectClass(
                ctx, js_traits<std::shared_ptr<T>>::QJSClassId
            );
            if (JS_IsException(jsobj))
                return jsobj;

            // shared_ptr with empty deleter since we don't own T*
            auto pptr = new std::shared_ptr<T>(
                ptr,
                [](T*)
                {
                }
        ...
        struct function
        {
            ...
            static function* create(JSRuntime* rt, Functor&& f)
            {
                ...
                if constexpr (!std::is_trivially_destructible_v<Functor_t>)
                {
                    ...
                    {
                        auto functorptr =
                            reinterpret_cast<Functor_t*>(fptr->functor);
        ...
        static void register_class(JSContext* ctx, const char* name)
        {
            ...
            if (e < 0)
                throw std::runtime_error{
                    "Cannot register C++ function class"
                };
        }
    };
    ...
    struct js_property_traits
    {
        static void set_property(
            JSContext* ctx, JSValue this_obj, Key key, JSValue value
        );
    ...
    {
        static void set_property(
            JSContext* ctx, JSValue this_obj, const char* name, JSValue value
        )
        {
        ...
        static JSValue get_property(
            JSContext* ctx, JSValue this_obj, const char* name
        ) noexcept
        {
    ...
    struct js_property_traits<std::string>
    {
        ...
        static JSValue get_property(
            JSContext* ctx, JSValue this_obj, const std::string& name
        ) noexcept
        {
    ...
    template <>
    struct js_property_traits<std::string_view>
    {
        static void set_property(
            JSContext* ctx,
            JSValue this_obj,
        ...
        static JSValue get_property(
            JSContext* ctx, JSValue this_obj, std::string_view name
        ) noexcept
        {
    ...
    {
        static void set_property(
            JSContext* ctx, JSValue this_obj, uint32_t idx, JSValue value
        )
        {
        ...
        static JSValue get_property(
            JSContext* ctx, JSValue this_obj, uint32_t idx
        ) noexcept
        {
    ...
    namespace detail
    {
        ...
        struct property_proxy
        {
            ...
            operator Value() const;  // defined later due to Value being
                                     // incomplete type
            ...
            property_proxy& operator=(T&& value)
            {
                js_property_traits<Key>::set_property(
                    ...
                    this_obj,
                    key,
                    js_traits<std::decay_t<T>>::wrap(
                        ctx, std::forward<T>(value)
                    )
                );
                return *this;
            }
     ...
     * A JSValue can be released by either JSValue x = std::move(value); or
     * JSValue x = value.release(), then the Value becomes invalid and FreeValue
     * won't be called Can be converted to C++ type, for example: auto string =
     * value.as<std::string>(); qjs::exception would be thrown on error
     * Properties can be accessed (read/write): value["property1"] = 1; value[2]
     * = "2";
     */
    class Value
    {
       ...
       public:
        ...
        /** Returns true if 2 values are the same (equality for arithmetic types
         * or point to the same object) */
        ...
        {
            if (ctx &&
                (ctx_holder.has_value() && !ctx_holder.value().expired()))
                JS_FreeValue(ctx, v);
        }
         ...
         * @return type returned by js_traits<std::decay_t<T>>::unwrap that
         * should be implicitly convertible to T
         * */
        template <typename T>
        auto as() const
        {
        ...
        /** Access JS properties. Returns proxy type which is implicitly
         * convertible to qjs::Value */
        template <typename Key>
        detail::property_proxy<Key> operator[](Key key)
        {
            assert(
                ctx && "Trying to access properties of Value with no JSContext"
            );
        ...
        {
            (*this)[name] = js_traits<decltype(std::function{ std::forward<
                Function>(f) })>::wrap(ctx, std::forward<Function>(f));
            return *this;
        }
        ...
        add(const char* name)
        {
            if constexpr (detail::get_set<M>::is_const::value)
                ...
                return add_getter_setter<
                    detail::get_set<M>::get,
        ...
        std::string toJSON(
            const Value& replacer = JS_UNDEFINED,
        ...
        )
        {
            ...
            return Value{
                weakFromContext(ctx),
                JS_EvalThis(
                    ctx, v, buffer.data(), buffer.size(), filename, flags
                )
            };
        }
    };
    ...
    class Runtime
    {
       public:
        ...
        /// @return pointer to qjs::Context of the executed job or nullptr if no
        /// job is pending
       ...
       private:
        static void promise_unhandled_rejection_tracker(
            JSContext* ctx,
            JSValue promise,
            JSValue reason,
            JS_BOOL is_handled,
    ...
    namespace detail
    {
        ...
        inline std::optional<std::string> readFile(
     ...
     * Calls JS_SetContextOpaque(ctx, this); on construction and JS_FreeContext
     * on destruction
     */
    class Context : public std::enable_shared_from_this<Context>
    {
       public:
        ...
        /** Module wrapper
         * Workaround for lack of opaque pointer for module load function by
         * keeping a list of modules in qjs::Context.
         */
        class Module
        {
           ...
           public:
            Module(JSContext* ctx, const char* name) : ctx(ctx), name(name)
            {
                m = JS_NewCModule(
                    ...
                    [](JSContext* ctx, JSModuleDef* m) noexcept
                    {
                        ...
                        // resort the exports to ensure that the shorter
                        // namespaces are processed before the longer ones, so
                        // that we can create nested objects correctly
                        std::ranges::sort(
                            it->exports,
                            [](const nvp& a, const nvp& b)
                            {
            ...
            {
                exports.push_back(
                    { std::string(name), { ctx, std::move(value) } }
                );
                JS_AddModuleExport(
            ...
            template <typename T>
            Module& add(std::string_view name, T&& value)
            {
                return add(
                    name, js_traits<T>::wrap(ctx, std::forward<T>(value))
                );
            }
            ...
            Module& function(const char* name, F&& f)
            {
                return add(
                    name,
                    js_traits<decltype(std::function{
                        std::forward<F>(f) })>::wrap(ctx, std::forward<F>(f))
                );
            }
           ...
           private:
            ...
            class class_registrar
            {
               ...
               public:
                ...
                /** Add a static member or function to the last added
                 * constructor. Example: struct T { static int var; static int
                 * func(); }
                ...
                class_registrar& static_fun(const char* name)
                {
                    ...
                    js_traits<qjs::shared_ptr<T>>::template ensureCanCastToBase<
                        F>(context.ctx);
                    ctor.add<F>(name);
                    return *this;
                }
                ...
                class_registrar& property(const char* name)
                {
                    ...
                    js_traits<std::shared_ptr<T>>::template ensureCanCastToBase<
                        FSet>(context.ctx);
                    if constexpr (std::
                                      is_same_v<decltype(FSet), std::nullptr_t>)
                 ...
                 * @param name constructor name (if not specified class name
                 * will be used)
                 */
                template <typename... Args>
                class_registrar& constructor(const char* name = nullptr)
                {
                    if (!name)
                        name = this->name;
                    ctor =
                        context.newValue(qjs::ctor_wrapper<T, Args...>{ name });
                ...
                {
                    static_assert(
                        !std::is_same_v<B, T>, "Type cannot be a base of itself"
                    );
                    assert(
                    ...
                    int err =
                        JS_SetPrototype(context.ctx, prototype.v, base_proto);
                 ...
                 * QuickJS garbage collector so that the cycle removal algorithm
                 * can find the other objects referenced by this object.
                 */
                template <Value T::* V>
       ...
       public:
        ...
        {
            return ModuleData{ detail::toUri(filename),
                               detail::readFile(filename) };
        };
        ...
        Value newObject()
        {
            return Value{ weakFromContext(ctx), JS_NewObject(ctx) };
        }
        ...
        )
        {
            ...
            JSValue v =
                JS_Eval(ctx, buffer.data(), buffer.size(), filename, flags);
        ...
        )
        {
            ...
            JSValue v = JS_EvalThis(
                ctx, this_obj.v, buffer.data(), buffer.size(), filename, flags
            );
            return Value{ weakFromContext(ctx), std::move(v) };
        }
        ...
        Value fromJSON(
            std::string_view buffer,
            const char* filename = "<fromJSON>",
            int flags = 0
        )
        {
            assert(
                buffer.data()[buffer.size()] == '\0' &&
                "fromJSON buffer is not null-terminated"
            );  // JS_ParseJSON requirement
            return Value{
                weakFromContext(ctx),
    ...
    /** Convert to/from std::function. Actually accepts/returns callable object
     * that is compatible with function<R (Args...)>.
    ...
    struct js_traits<std::function<R(Args...)>, int>
    {
        static auto unwrap(JSContext* ctx, JSValueConst fun_obj)
        {
            ...
            return [jsfun_obj = Value{ weakFromContext(ctx),
                                       JS_DupValue(ctx, fun_obj) },
                    weak](Args... args) -> R
            {
                if (weak.expired())
                ...
                auto work = [&]()
                {
                    ...
                    detail::wrap_args(
                        jsfun_obj.ctx, argv, std::forward<Args>(args)...
                    );
        ...
        static JSValue wrap(JSContext* ctx, Functor&& functor) noexcept
        {
            ...
            try
            {
                auto fptr = function::create(
                    JS_GetRuntime(ctx), std::forward<Functor>(functor)
                );
                fptr->invoker = [](function* self,
                                   JSContext* ctx,
                                   JSValueConst this_value,
                                   int argc,
                                   JSValueConst* argv)
                {
                    assert(self);
                    auto f = reinterpret_cast<std::decay_t<Functor>*>(
                        &self->functor
                    );
    ...
    struct js_traits<
        Function,
        std::enable_if_t<detail::is_callable_v<Function>>>
    {
        static auto unwrap(JSContext* ctx, JSValueConst fun_obj)
        {
            return js_traits<
    ...
    /** Convert from std::vector<T> to Array and vice-versa. If Array holds
     * objects that are non-convertible to T throws qjs::exception */
    template <class T>
    ...
    struct js_traits<std::pair<U, V>>
    {
        ...
        static std::pair<U, V> unwrap(JSContext* ctx, JSValueConst jsarr)
        {
            ...
            if (e == 0)
                JS_ThrowTypeError(
                    ctx,
                    "js_traits<%s>::unwrap expects array",
            ...
            if (len != 2)
            {
                JS_ThrowTypeError(
                    ctx,
                    "js_traits<%s>::unwrap expected array of length 2, got "
                    "length %d",
                    QJSPP_TYPENAME(std::pair<U, V>),
                    len
                );
    ...
    template <typename... T>
    struct js_traits<std::tuple<T...>>
    {
        static JSValue wrap(
            JSContext* ctx, const std::tuple<T...>& obj
        ) noexcept
        {
            try
            {
       ...
       private:
        template <size_t... I>
        static std::tuple<T...> unwrap_impl(
            JSContext* ctx, JSValueConst jsarr, std::index_sequence<I...>
        )
        {
            try
            {
                return std::make_tuple(
                    js_traits<std::tuple_element_t<I, std::tuple<T...>>>::
                        unwrap(ctx, JS_GetPropertyUint32(ctx, jsarr, I))...
                );
            }
            catch (...)
            {
    ...
    struct js_traits<std::optional<T>>
    {
        ...
        static auto unwrap(JSContext* ctx, JSValueConst v) noexcept
            -> std::optional<
                decltype(js_traits<std::decay_t<T>>::unwrap(ctx, v))>
        {
            try
            {
    ...
    inline void Runtime::promise_unhandled_rejection_tracker(
        JSContext* ctx,
        JSValue promise,
        JSValue reason,
        JS_BOOL is_handled,
    ...
    inline JSModuleDef* Runtime::module_loader(
        JSContext* ctx, const char* module_name, void* opaque
    )
    {
        ...
        try
        {
            ...
            if (!data.source)
            {
                JS_ThrowReferenceError(
                    ctx, "could not load module filename '%s'", module_name
                );
                return NULL;
            }
